<div class="review__container">
    @if($user->userCourseReviews->count())
        <div class="reviews_items">
            @foreach($user->userCourseReviews as $review)
                <div class="review_item">

                    <div class="review__top_side">
                        <div class="review__left_side">
                            <p class="pre__review_name">Ваш Отзыв Курсу</p>
                            <a href="/course/{{ $review->id_course }}" target="_blank" class="review__name">{{ $review->course->name }}</a>
                        </div>
                    ...
                    @else
                        ...
                        <br>
                        <a href="{{ route(userRoute('review_delete'), ['id' => $review->id]) }}?type=course" class="btn btn2 confirm__item">Удалить</a>
                    @endif
                </div>
            @endforeach
        </div>
